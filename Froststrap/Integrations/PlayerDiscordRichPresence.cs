...
namespace Froststrap.Integrations
{
    internal class PlayerDiscordRichPresence : IDisposable
    {
        ...
        public PlayerDiscordRichPresence(ActivityWatcher activityWatcher)
        {
            ...
            _rpcClient.OnReady += (_, e) => App.Logger.Info($"Received ready from user {e.User} ({e.User.ID})");

            _rpcClient.OnPresenceUpdate += (_, e) =>
                App.Logger.Info("Presence updated");

            _rpcClient.OnError += (_, e) =>
                App.Logger.Error($"An RPC error occurred - {e.Message}");

            _rpcClient.OnConnectionEstablished += (_, e) =>
                App.Logger.Info("Established connection with Discord RPC");

            _rpcClient.OnClose += (_, e) =>
                App.Logger.Error($"Lost connection to Discord RPC - {e.Reason} ({e.Code})");
        ...
        public void ProcessRPCMessage(Message message, bool implicitUpdate = true)
        {
            ...
            {
                App.Logger.Info("Presence is not set, enqueuing message");
        ...
        private void ProcessSetRichPresence(Message message, bool implicitUpdate)
        {
            ...
            {
                App.Logger.Error("Failed to parse message! (JSON deserialization threw an exception)");
                return;
            }
            ...
            if (presenceData.Details is not null)
            {
                if (presenceData.Details.Length > 128)
                    App.Logger.Error($"Details cannot be longer than 128 characters");
            ...
            if (presenceData.State is not null)
            {
                if (presenceData.State.Length > 128)
                    App.Logger.Error($"State cannot be longer than 128 characters");
        ...
        public async Task<bool> SetCurrentGame()
        {
            ...
            {
                App.Logger.Info("Not in game, clearing presence");
            ...
            {
                App.Logger.Info("Processing queued messages");
            ...
            try
            {
                if (_visible)
                {
                    if (_currentPresence != null)
                    {
                        ...
                        App.Logger.Info("Updating presence");
                        _rpcClient.SetPresence(_currentPresence);
                    }
                    else
                    {
                        App.Logger.Info("Clearing presence (no current presence)");
                        _rpcClient.ClearPresence();
                    }
                }
                else
        ...
        public void Dispose()
        {
            ...
            App.Logger.Info("Cleaning up Discord RPC and Presence");
